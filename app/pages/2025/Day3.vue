<template>{{ solve(text) }}</template>
<script lang="ts" setup>
const data = ref('')
onMounted(async()=>{
  data.value = (await(await fetch('/inputExample/2025/day3.txt')).text())
...
function findMoreThanRestInWord(current:string, text:string, index = 0) {
...
function wantToPick(len:number) {
  return function selectiveMostWord(word:string, collected = [] as string[]) {
